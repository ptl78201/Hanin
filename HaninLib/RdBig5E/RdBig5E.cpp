// RdBig5E.cpp : Defines the entry point for the application.
//
...
short	GetBig5EChar (char *pCmp, char *pCharBuf);
...
short	ReadBig5E (char	*big5e_dic)
{
	...
	while (fscanf (fp, "%s", szRead) != EOF)
	{
		...
		{
#ifndef	_MAC_
			sprintf (szBuf, "Error 1 in line %d", nLine);
			MessageBox (NULL, szBuf, "RdBig5E", MB_OK);
#endif
			continue;
		}
		...
		if (szCurZhuin[0] == 0 || strcmp (szCurZhuin, &szRead[2]) != 0)
		{
			...
			{
#ifndef	_MAC_
				sprintf (szBuf, "Idx full : line %d", nLine);
				MessageBox (NULL, szBuf, "RdBig5E", MB_OK);
#endif
				continue;
			}
			...
			{
#ifndef	_MAC_
				sprintf (szBuf, "Error 2 in line %d", nLine);
				MessageBox (NULL, szBuf, "RdBig5E", MB_OK);
#endif
				continue;
			}
			else if (nIdxLen > 0)
			{
				...
				{
#ifndef	_MAC_
					sprintf (szBuf, "Error 3 in line %d", nLine);
					MessageBox (NULL, szBuf, "RdBig5E", MB_OK);
#endif
					continue;
				}
...
#ifndef	_MAC_
	sprintf (szBuf, "Total line %d", nLine);
	MessageBox (NULL, szBuf, "RdBig5E", MB_OK);
#endif
	fclose (fp);
	return 1;
